...
def test_reconcile_reregisters_on_url_drift() -> None:
    ...
    assert sup.snapshot().last_reconcile_outcome == "healed:url_mismatch"
...
def test_reconcile_reregisters_when_url_unset() -> None:
    ...
    asyncio.run(sup.reconcile())
    bot.set_webhook.assert_awaited_once()
    assert sup.snapshot().last_reconcile_outcome == "healed:url_unset"
...
def test_reconcile_heals_on_pending_backlog() -> None:
    bot = _bot(_webhook_info(pending=60))
    sup = _supervisor(bot)
    asyncio.run(sup.reconcile())
    bot.set_webhook.assert_awaited_once()
    assert sup.snapshot().last_reconcile_outcome == "healed:pending_backlog"
...
def test_reconcile_heals_on_recent_delivery_errors() -> None:
    ...
    asyncio.run(sup.reconcile())
    bot.set_webhook.assert_awaited_once()
    assert sup.snapshot().last_reconcile_outcome == "healed:delivery_errors"
...
def test_stale_with_symptom_triggers_heal() -> None:
    ...
    heartbeat = Heartbeat(clock=clock)
    clock.advance(5000)
    bot = _bot(_webhook_info(pending=3))  # below backlog threshold, but stale + pending
    sup = _supervisor(bot, heartbeat=heartbeat, clock=clock)
    asyncio.run(sup.reconcile())
    bot.set_webhook.assert_awaited_once()
    assert sup.snapshot().last_reconcile_outcome == "healed:stale_with_symptom"
...
def test_half_open_breaker_allows_one_trial() -> None:
    ...
    asyncio.run(sup.reconcile())
    bot.set_webhook.assert_awaited_once()
    assert breaker.state is BreakerState.CLOSED  # trial succeeded
