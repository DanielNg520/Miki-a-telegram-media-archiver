...
- **Count** — after every _N_ media messages (`periodic_notice_media_threshold`, default 10; an album counts as one message), then a ~5s quiet gap so the reminder posts once the batch finishes rather than mid-send.
